...
if $test_opensc_status >/dev/null; then
  ...
  if modutil -dbdir "sql:${HOME}/.pki/nssdb/" -list | grep "CAC Module";
  then
    echo "'CAC Module' already installed";
    exit 0
  fi

  echo "Adding CAC Module libs to ${HOME}/.pki/nssdb/"
  # Add OpenSC as a new PKCS #11 library to the security database
  modutil -dbdir "sql:${HOME}/.pki/nssdb/" -add "CAC Module" -libfile "$($opensc_pkcs11_lib)"
